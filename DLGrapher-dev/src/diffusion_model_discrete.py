...
class DiscreteDenoisingDiffusion(pl.LightningModule):
    def __init__(self, cfg, dataset_infos, train_metrics, sampling_metrics, visualization_tools, extra_features,
                 domain_features, compress: Optional[Compress]=None, tab_gen: Optional[TAB_GEN_TYPE]=None):
        super().__init__()
        ...
        self.train_loss = TrainLossDiscrete(cfg.model.lambda_train)
    ...
    def validation_step(self, data, i):
        ...
        extra_data = self.compute_extra_data(noisy_data)
        pred = self(noisy_data, extra_data, node_mask)
        self.train_loss(masked_pred_X=pred.X, masked_pred_E=pred.E, pred_y=pred.y,
                               true_X=dense_data.X, true_E=dense_data.E, true_y=data.y,
                               log=False)
        nll = self.compute_val_loss(pred, noisy_data, dense_data.X, dense_data.E, data.y,  node_mask, test=False)
        return {'loss': nll}

    def on_validation_epoch_end(self) -> None:
        metrics = [self.metric_nll.compute(), self.val_X_kl.compute() * self.T, self.val_E_kl.compute() * self.T,
    ...
    def on_test_epoch_end(self) -> None:
        ...
        metrics = [self.metric_nll.compute(), self.test_X_kl.compute(), self.test_E_kl.compute(),
        ...
        if self.cfg.general.write_edgelist:
            time_str = time.strftime('%Y-%m-%d-%H-%M-%S')
            ds_name = self.cfg.dataset.name
            dir_path = os.path.join(
                os.getcwd(), f"digress_{ds_name}_{time_str}_{self.current_epoch}e")
            os.mkdir(dir_path)
            for i, (_nodes, edges) in enumerate(samples):
                nx.write_edgelist(
                    nx.from_numpy_array(edges.numpy()),
                    os.path.join(dir_path, f'digress_{ds_name}_{i}.txt'), data=False)
            ...
            with open(os.path.join(dir_path, 'info.json'), 'w') as f:
                json.dump({
                    ...
                    'sampling_time': self.test_sample_time
                }, f)
    ...
    def compute_val_loss(self, pred, noisy_data, X, E, y, node_mask, test=False):
       ...
       """
        ...
        loss_term_0 = X_logp(X * prob0.X.log()) + E_logp(E * prob0.E.log())
    ...
    def sample_batch(self, batch_id: int, batch_size: int, keep_chain: int, number_chain_steps: int,
        ...
        for i in range(batch_size):
            ...
            if self.compress:
                node_types, edge_types = self.compress.decompress(
                    torch.stack((node_types, torch.ones_like(node_types)), dim=-1),
                    edge_types)
                node_types = node_types.squeeze()
                n = node_types.size(0)

            if self.tab_gen:
                node_data = self.tab_gen.sample(n.item())
            else:
                node_data = node_types.numpy(force=True)

            sample_list.append((node_data, edge_types.numpy(force=True)))
    ...
    def sample_p_zs_given_zt(self, s, t, X_t, E_t, y_t, node_mask):
        ...
        out_one_hot = utils.PlaceHolder(X=X_s, E=E_s,
                                        y=torch.zeros(y_t.shape[0], 0, device=self.device))
    ...
    def save_samples(self, samples: list[tuple[np.ndarray, np.ndarray]]) -> None:
        i = 1
        ext = "pickle" if self.tab_gen else "txt"
