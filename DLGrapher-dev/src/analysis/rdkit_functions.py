...
class BasicMolecularMetrics(object):
    def __init__(self, dataset_info, device: str, train_smiles=None):
    ...
    def evaluate(self, generated):
        ...
        # TODO confirm min number of synthetic samples and
        # raise specific error if lower
        try:
            fcd = FCD()(cast(list[str], self.dataset_smiles_list), cast(list[str], valid))
        except ValueError:
            fcd = float('inf')

        print(f"FCD: {fcd:.3f}")

        return ([validity, relaxed_validity, uniqueness, novelty, fcd], unique,
...
def compute_molecular_metrics(molecule_list, train_smiles, dataset_info, device="cpu"):
    """ molecule_list: (dict) """
    ...
    metrics = BasicMolecularMetrics(dataset_info, device, train_smiles)
    ...
    if wandb.run:
        ...
        dic = {'Validity': rdkit_metrics[0][0], 'Relaxed Validity': rdkit_metrics[0][1],
               ...
               'FCD': rdkit_metrics[0][4], 'nc_max': nc['nc_max'], 'nc_mu': nc['nc_mu']}
        wandb.log(dic)
