...
from src.analysis.dist_helper import compute_mmd, gaussian_emd, gaussian, emd, gaussian_tv
from torch_geometric.utils import to_networkx
import wandb

PRINT_TIME = False
__all__ = ['degree_stats', 'clustering_stats', 'orbit_stats_all', 'spectral_stats']
...
def degree_stats(graph_ref_list, nonempty_graph_pred_list, is_parallel=True, compute_emd=False):
    ...
    if is_parallel:
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            for deg_hist in executor.map(degree_worker, nonempty_graph_pred_list):
...
def spectral_stats(graph_ref_list, nonempty_graph_pred_list, is_parallel=True, n_eigvals=-1, compute_emd=False):
    ''' Compute the distance between the degree distributions of two unordered sets of graphs.
        Args:
            graph_ref_list, graph_target_list: two lists of networkx graphs to be evaluated
        '''
    ...
    if is_parallel:
        with concurrent.futures.ThreadPoolExecutor() as executor:
            for spectral_density in executor.map(spectral_worker, graph_ref_list, [n_eigvals for _ in graph_ref_list]):
                sample_ref.append(spectral_density)
        with concurrent.futures.ThreadPoolExecutor() as executor:
            for spectral_density in executor.map(spectral_worker, nonempty_graph_pred_list,
        ...
        for i in range(len(graph_ref_list)):
            ...
            sample_ref.append(spectral_temp)
        for i in range(len(nonempty_graph_pred_list)):
            spectral_temp = spectral_worker(nonempty_graph_pred_list[i], n_eigvals)
            sample_pred.append(spectral_temp)
    ...
    if is_parallel:
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            for clustering_hist in executor.map(
                    clustering_worker, [(G, bins) for G in nonempty_graph_pred_list]):
        ...
        for i in range(len(nonempty_graph_pred_list)):
            clustering_coeffs_list = list(
                nx.clustering(nonempty_graph_pred_list[i]).values())
            hist, _ = np.histogram(
                clustering_coeffs_list, bins=bins, range=(0.0, 1.0), density=False)
...
def motif_stats(graph_ref_list, nonempty_graph_pred_list, motif_type='4cycle', ground_truth_match=None,
    ...
    for G in nonempty_graph_pred_list:
        orbit_counts = orca(G)
        motif_counts = np.sum(orbit_counts[:, indices], axis=1)
...
def orbit_stats_all(graph_ref_list, graph_pred_list, compute_emd=False):
    ...
    # ]
    # TODO check whether to also use list with empty graphs removed here
...
class SpectreSamplingMetrics(nn.Module):
    ...
    def forward(self, generated_graphs: list, name, current_epoch, val_counter, local_rank, test=False):
        ...
        for _node_types, edge_types in generated_graphs:
            A = edge_types.numpy().astype(bool)
        ...
        nonempty_networkx_graphs = [G for G in networkx_graphs if G.number_of_nodes()]

        if 'degree' in self.metrics_list:
            if local_rank == 0:
                print("Computing degree stats..")
            degree = degree_stats(
                reference_graphs, nonempty_networkx_graphs,
                is_parallel=True, compute_emd=self.compute_emd)
            to_log['mmd/degree'] = degree
            if wandb.run:
        ...
        if 'spectre' in self.metrics_list:
            ...
            spectre = spectral_stats(
                reference_graphs, nonempty_networkx_graphs,
                is_parallel=True, n_eigvals=-1, compute_emd=self.compute_emd)
            to_log['mmd/spectre'] = spectre
            if wandb.run:
        ...
        if 'clustering' in self.metrics_list:
            ...
            clustering = clustering_stats(
                reference_graphs, nonempty_networkx_graphs,
                bins=100, is_parallel=True, compute_emd=self.compute_emd)
            to_log['mmd/clustering'] = clustering
            if wandb.run:
        ...
        if 'motif' in self.metrics_list:
            ...
            motif = motif_stats(
                reference_graphs, nonempty_networkx_graphs,
                motif_type='4cycle', ground_truth_match=None, bins=100, compute_emd=self.compute_emd)
            to_log['mmd/motif'] = motif
            if wandb.run:
        ...
        if 'planar' in self.metrics_list:
            ...
            to_log['sampling/planar_acc'] = planar_acc
            if wandb.run:
