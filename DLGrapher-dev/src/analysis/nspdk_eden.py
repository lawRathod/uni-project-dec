...
def kernel_compute(X, Y=None, metric='linear', n_jobs=None):
    X = vectorize(X, complexity=4, discrete=True)

    if Y is not None:
        Y = vectorize(Y, complexity=4, discrete=True)

    return pairwise_kernels(X, Y, metric, n_jobs=n_jobs)


def nspdk_stats(graph_ref_list, nonempty_graph_pred_list, n_jobs=None):
    X = kernel_compute(graph_ref_list, n_jobs=n_jobs)
    Y = kernel_compute(nonempty_graph_pred_list, n_jobs=n_jobs)
    Z = kernel_compute(
        X=graph_ref_list, Y=nonempty_graph_pred_list, n_jobs=n_jobs)
    mmd_dist = np.average(X) + np.average(Y) - 2 * np.average(Z)

    return mmd_dist


class NSPDKSamplingMetrics(nn.Module):
    def __init__(self, datamodule, node_target_ind=-1, edge_target_ind=-1):
        super().__init__()

        self.node_target_ind = node_target_ind
        self.edge_target_ind = edge_target_ind
        self.val_graphs = self.dataset_to_nx(datamodule.val_dataset)
        self.test_graphs = self.dataset_to_nx(datamodule.test_dataset)
        self.num_graphs_test = len(self.test_graphs)
        self.num_graphs_val = len(self.val_graphs)

    def dataset_to_nx(self, dataset):
        networkx_graphs = [nx.Graph()] * len(dataset)

        for i, d in enumerate(dataset):
            d_ = d.clone()

            if self.edge_target_ind == -1:
                d_.label = d.edge_attr.argmax(dim=-1)
            else:
                d_.label = d.edge_attr[:, self.edge_target_ind]

            g =  to_networkx(
                d_, node_attrs=None, edge_attrs=['label'],
                to_undirected=True, remove_self_loops=True)
            networkx_graphs[i] = g

            if self.node_target_ind == -1:
                x = d.x.argmax(dim=-1)
            else:
                x = d.x[:, self.node_target_ind]

            nx.set_node_attributes(
                g, {n: {'label': label} for n, label in enumerate(x.numpy())})
    ...
    def forward(self, generated_graphs: list, local_rank, test=False):
        ...
        networkx_graphs: list[nx.Graph] = [nx.Graph()] * len(generated_graphs)

        if local_rank == 0:
            print('Building labeled networkx graphs...')

        for i, (node_types, edge_types) in enumerate(generated_graphs):
            if self.node_target_ind == -1:
                x = node_types
            else:
                x = node_types[:, self.node_target_ind]

            if self.edge_target_ind == -1:
                adj = edge_types
            else:
                adj = edge_types[:, :, self.edge_target_ind]

            g = nx.from_numpy_array(adj.numpy(), edge_attr='label')
            nx.set_node_attributes(
                g, {n: {'label': label} for n, label in enumerate(x.numpy())})
            networkx_graphs[i] = g

        nspdk = nspdk_stats(
            reference_graphs, [G for G in networkx_graphs if G.number_of_nodes()])

        if wandb.run:
            wandb.run.summary['nspkd'] = nspdk

        if local_rank == 0:
            print("NSPDK:", nspdk)
        if wandb.run:
            wandb.log({'mmd/nspdk': nspdk}, commit=False)

###

# https://github.com/harryjo97/GDSS/blob/24cc490e0c5b39cbd265fda33ab2bc8b528d4454/evaluation/eden.py
