...
from src.analysis.nspdk_eden import NSPDKSamplingMetrics
...
class SamplingMolecularMetrics(nn.Module):
    def __init__(self, dataset_infos, train_smiles, datamodule=None):
        super().__init__()
        di = dataset_infos
        ...
        self.nspdk_sampling_metrics = NSPDKSamplingMetrics(datamodule=datamodule) if datamodule else None
    ...
    def forward(self, molecules: list, name, current_epoch, val_counter, local_rank, test=False):
        ...
        if self.nspdk_sampling_metrics:
            self.nspdk_sampling_metrics(molecules, local_rank, test)
