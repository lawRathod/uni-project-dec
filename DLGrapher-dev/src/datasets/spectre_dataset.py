...
class SpectreGraphDataset(InMemoryDataset):
    def __init__(self, dataset_name, split, root,
                 compress: Optional[Compress], num_graphs: int, seed: int,
    ...
    def processed_file_names(self):
        suffix = '_compress' if self.compress else ''
        return [self.split + suffix + '.pt']
    ...
    def download(self):
        ...
        g_cpu = torch.Generator().manual_seed(self.seed)
...
class SpectreGraphDataModule(AbstractDataModule):
    def __init__(self, cfg, compress: Optional[Compress], num_graphs=200):
        # self.cfg = cfg
        base_path = pathlib.Path(os.path.realpath(__file__)).parents[2]
        root_path = os.path.join(base_path, cfg.dataset.datadir)

        datasets = {'train': SpectreGraphDataset(dataset_name=cfg.dataset.name,
                                                 split='train', root=root_path,
                                                 compress=compress,
                                                 num_graphs=num_graphs,
                                                 seed=cfg.dataset.seed),
                    'val': SpectreGraphDataset(dataset_name=cfg.dataset.name,
                                               split='val', root=root_path,
                                               compress=compress,
                                               num_graphs=num_graphs,
                                               seed=cfg.dataset.seed),
                    'test': SpectreGraphDataset(dataset_name=cfg.dataset.name,
