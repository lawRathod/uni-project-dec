...
def create_inductive_split_custom(dataset_name, normalize=True):
    ...
    def combine_subgraphs(graph_list, label_prefix="combined"):
        ...
        combined_edges = torch.cat(all_edges, dim=1) if all_edges else torch.empty((2, 0), dtype=torch.long)
        combined_mask = torch.ones(combined_x.shape[0], dtype=torch.bool)
        
        # Ensure edge indices are valid for the number of nodes
        if combined_edges.shape[1] > 0:
            max_node_idx = combined_edges.max().item()
            if max_node_idx >= combined_x.shape[0]:
                print(f"Warning: Edge index {max_node_idx} >= num nodes {combined_x.shape[0]}, clipping edges")
                valid_edge_mask = (combined_edges[0] < combined_x.shape[0]) & (combined_edges[1] < combined_x.shape[0])
                combined_edges = combined_edges[:, valid_edge_mask]
